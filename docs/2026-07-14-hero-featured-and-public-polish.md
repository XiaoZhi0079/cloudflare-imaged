...
日期：2026-07-14
状态：近似 `16:9` 精选、后台职责拆分、加载失败保护与 D1 热路径根治均已发布至 `gallery.140079.xyz`
更新：2026-07-16
...
> 本节保留 2026-07-15 初版生产行为与验收证据；当前精选资格已由 2.10 的近似比例规则取代。
...
### 2.10 近似 `16:9` 精选资格（已发布）

2026-07-16 完成当前轮播资格放宽：

- 图片宽高比与 `16:9` 的相对偏差允许落在 `±0.5%` 闭区间内，对应比例约为 `1.76889–1.78667`。共享分类器使用 `BigInt` 交叉乘法判断，不依赖浮点除法。
- 最低分辨率由 `1920×1080` 调整为 `1600×900`。`1600×900` 和用户现有的 `1672×941` 素材均可用；`1280×720` 仍因分辨率不足而不可用。
- `isExactSixteenNine` 保持原语义，并新增 `isApproximatelySixteenNine`。因此 `1672×941` 的精确比例字段为 `false`，近似比例字段和最终资格均为 `true`。
- 内部最低档位键继续使用 `1k` 保持兼容，管理端可见名称改为 `HD+ / 900p+`；2K 与 4K 继续使用既有宽高双门槛。
- Repository 与管理 API 继续复用共享分类器并保持原子校验；不合格 ID 会以新规则文案拒绝，站点文字和现有精选顺序不会被部分写入。
- 设置页入口和 `site-settings.js` 依赖统一使用缓存版本 `20260716-approximate-featured-ratio`；未变更的设置 CSS 与图片库资产继续沿用 `20260715-featured-filter-separation`，避免无意义的缓存失效。
- D1 中既有 `images.width` / `images.height` 继续作为权威元数据，本次不新增字段、不运行 migration，也不重新读取图片内容。
- Hero 舞台继续保持精确 `16:9` 并使用 `object-fit: contain`。近似比例图片会完整展示、不拉伸、不裁切，最多只产生极细的留边。
...
- 精选控制器区分“配置已载入”和“请求进行中”：只有 `GET /api/admin/site` 成功并完整应用响应后，本期名字、大屏文案、选图和保存控件才会解锁。
- HTML 初始态同步禁用上述控件，避免模块尚未初始化时出现短暂可操作窗口；保存函数还会再次校验就绪状态，程序化触发也不能绕过保护。
- 非鉴权加载异常会保留页面框架、显示安全错误和“重新加载”入口；失败状态不会请求图片库，也不会发送 `PATCH /api/admin/site`。重试成功后才恢复编辑。
- 401 继续清除本地管理密钥并返回登录界面，不进入应用内重试状态；重试进行中暂时禁用退出，退出操作会使尚未完成的旧请求结果失效，避免异步请求重新显示应用。
- 精选 HTML、入口和控制器链升级为 `20260716-featured-load-guard`；未变更的标签分类入口、设置 CSS 和图片库资产保持现有缓存版本。
- 本次不修改 API、Repository、D1、migration、公开页面或图片资格规则；验收只使用合成数据和静态 DOM，未访问任何图库图片内容。
- 本地验收：2 个变更 JavaScript 文件全部通过 `node --check`；3 个重点测试文件为 35/35 通过；`npm test` 为 181/181 通过。
- 生产发布：功能提交 `34c39ef` 已推送至 GitHub；CI run `29513832298` 成功，Cloudflare Pages Production 部署 `c52447c9-dc08-45e7-9b16-ba903acdc80a` 使用同一来源提交。自定义域名和部署 URL 均返回新缓存链、初始锁定与重试防线，无密钥管理站点请求返回 401。
- 生产验收只请求主页/后台 HTML、两个管理 JavaScript 文件和未授权 API；未执行页面脚本，未请求、下载、解码、截图或分析任何图库图片。
...
- [x] 最终发布提交全量测试通过（181 / 181），GitHub CI 与 Cloudflare Pages 部署成功
- [x] 生产标签 API TTFB 中位数 `128ms`，浏览器标签切换完成于 `150–190ms`

### 2026-07-17 多图集与统一视觉改版

- “精选管理”升级为“图集管理”，图集拥有名字、介绍、封面、首页状态和有序图片成员；一张图片可属于多个图集。
- 新增 `albums` / `album_images` D1 结构与管理员/公开 API；首页主图集中的轮播合规成员继续驱动 Hero。
- 前台恢复安全的图片文件名展示，新增图集列表和图集详情页，并依据根目录 `参考.html` 统一为中性灰白视觉体系。
- 图片库详情抽屉改为桌面宽屏双栏、移动端全屏单栏，预览不再强制 4:3。
- 生产 `0002_albums.sql` 已应用：迁移后 1 个首页图集、5 个图集成员，与 5 条旧精选记录一致；无待执行迁移。
- 验收不请求、下载、解码、截图或分析图库图片，只使用合成测试、匿名计数和线上文本资源。
