...
日期：2026-07-14
状态：大屏精选基线已发布至 `gallery.140079.xyz`；精选尺寸资格与固定 `16:9` 改造已在功能分支实施并本地验证，尚未推送或部署
更新：2026-07-15
...
### 2.7 精选尺寸资格与固定 `16:9` Hero（已实施并验证，尚未推送/部署）

2026-07-15 在功能分支完成以下改造：

- 精选资格使用单一共享规则：图片必须为**精确 `16:9` 且至少 `1920×1080`**；只有精确 `3840×2160` 标记为“4K”，包括 `7680×4320` 在内的其他尺寸不显示“4K”标签。
- 管理图片库在卡片和详情中显示尺寸、轮播资格及不可用原因，并提供互斥的“全部 / 轮播可用 / 4K”筛选；筛选继续与搜索、标签和选择状态组合。
- 站点精选选择器显示相同的尺寸状态并禁选新的不合规图片；当前列表中的旧不合规项保留警告和“移除”操作。
...
- 前台 `main.css` / `gallery.js` 与本次改动的管理端 `workbench.css`、`library-page.js`、`library-state.js`、`renderers/image-card.js`、`settings.css`、`settings-page.js`、`site-settings.js` 统一使用缓存版本 `20260715-featured-dimensions`；未改依赖和全局缓存头保持不变。
- 部署前已经存在于生产 `featured_images` 的旧不合规精选**不会被读取路径自动过滤或删除**，公开站点仍会返回并完整展示。用户需要在管理端手动移除；移除后服务端不允许再次加入不合规图片。

本地验证证据：

- `npm test`：157 项测试，157 项通过，0 项失败。
- 共享规则、管理端入口/状态/渲染/设置模块、管理 API 共享模块与 Repository 的 `node --check` 全部退出 0。
- `git diff --check` 退出 0。
- 验证过程未查看或分析任何图片，未访问或写入生产 D1/R2，未推送、部署或截图。
...
2. **生产库 schema 同步（已完成）**
   - [x] 首次请求通过 repository bootstrap 创建/确认新表与默认文案
   - [x] `/api/public/site` 返回 JSON，当前默认期名为「图集」、精选数为 0
   - [x] 既有标签与图片读取正常；未修改现有图片、标签或 R2 对象
...
- [x] 标签浏览流程不受影响（自动测试）
- [x] 精选仅允许精确 `16:9` 且至少 `1920×1080`；仅精确 `3840×2160` 标记为“4K”
- [x] 管理图片库可标记/筛选资格，精选选择器禁选新的不合规图片
- [x] 旧不合规精选保持只读兼容并可由用户手动移除
- [x] Hero 使用响应式 `16:9` + `contain`；移动端透明文案在舞台下正常流
- [x] 当前功能分支全量测试通过（157 / 157；本改造尚未推送或部署）
...
- 生产仅执行公开/管理未授权只读请求以及运行时 schema/default bootstrap；未执行管理写操作或 R2 写入。
