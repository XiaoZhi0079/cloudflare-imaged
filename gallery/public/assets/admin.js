import { renderAdminImageGrid, renderAdminImageList, renderAdminTagList, renderAdminTagRail } from "./templates.js";
...
const tagFilterSelect = $("#admin-tag-filter");
...
function setConnected(connected) {
  ...
  setDisabled(tagFilterSelect, !connected);
  ...
  if (tagList) {
    if (isLibraryPage()) {
      tagList.innerHTML = tags.length
        ? renderAdminTagRail(tags, activeTagFilter)
        : emptyState("\u8fd8\u6ca1\u6709\u4efb\u4f55\u6807\u7b7e\u3002");
      bindTagRailActions();
    } else {
      tagList.innerHTML = tags.length
...
async function runTagAction(tagId, action) {
  ...
  if (action === "assign-order") {
    ...
    await updateTag(tagId, { sortOrder });
    await loadTags();
    setStatus(`\u5df2\u66f4\u65b0\u6392\u5e8f\uff1a${current.name}`, "success");
    return;
  }

  if (action === "toggle-tag") {
    await updateTag(tagId, { isVisible: !current.isVisible });
    await loadTags();
    setStatus(`\u5df2${current.isVisible ? "\u9690\u85cf" : "\u663e\u793a"}\u6807\u7b7e\uff1a${current.name}`, "success");
    return;
  }
  ...
  if (action === "delete-tag") {
    ...
    await loadTags();
    setStatus(`\u5df2\u5220\u9664\u6807\u7b7e\uff1a${current.name}`, "success");
  }
}
...
function bindTagRailActions() {

  if (!tagList) {
    return;
  }

  tagList.querySelectorAll("[data-tag-filter]").forEach((button) => {
    button.addEventListener("click", () => {
      activeTagFilter = activeTagFilter === button.dataset.tagFilter ? "" : button.dataset.tagFilter;
      renderTagFilterOptions();
      renderTags();
      renderImages();
    });
...
function renderTagFilterOptions() {
  ...
  if (activeTagFilter && !tags.some((tag) => tag.name === activeTagFilter)) {
    activeTagFilter = "";
  }

  tagFilterSelect.innerHTML = `<option value="">\u5168\u90e8\u6807\u7b7e</option>${tags
    .map((tag) => `<option value="${escapeHtml(tag.name)}" ${tag.name === activeTagFilter ? "selected" : ""}>${escapeHtml(tag.name)}</option>`)
    .join("")}`;
  tagFilterSelect.value = activeTagFilter;
  setDisabled(tagFilterSelect, !isConnected);
}
...
function getFilteredImages() {
  ...
  return images.filter((image) => {
    ...
    const matchesTag = !activeTagFilter || imageTags.includes(activeTagFilter);
...
function initUploadPage() {
  ...
  uploadSubmitButton?.addEventListener("click", () => {
    runButtonAction(uploadSubmitButton, async () => {
      ...
      const payload = await fetchJson("/api/admin/images/upload", {
        method: "POST",
        body: formData,
      });
